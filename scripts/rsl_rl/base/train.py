...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../source/soccerTask")))
import cli_args
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if agent_cfg.resume or agent_cfg.algorithm.class_name == "Distillation":
        try:
            ...
            if agent_cfg.algorithm.class_name == "Distillation":
                raise
            print(
                "[WARN] Resume checkpoint not found. Falling back to fresh training run. "
                f"load_run={agent_cfg.load_run}, checkpoint={agent_cfg.load_checkpoint}."
            )
            print(f"[WARN] Reason: {err}")
    ...
    # load the checkpoint
    if resume_path is not None and (agent_cfg.resume or agent_cfg.algorithm.class_name == "Distillation"):
