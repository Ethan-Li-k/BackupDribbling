...
class RewardsCfg:
    ...
    # Joint Position (Reference Motion) - Duplicate handled below
    # joint_pos = RewTerm(
    ...
    feet_distance = RewTerm(
        ...
        params={
            ...
            "max_dist": 0.45, # Reduced from 0.60 to avoid wide stance
            "asset_cfg": SceneEntityCfg("robot", body_names=".*_foot_link"),
        },
    )
    ...
    # Forward Velocity (Discourage tiny shuffling)
    ...
    torques = RewTerm(
        ...
        weight=-1e-5, # Increased penalty (from -5e-6)
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    ...
    dof_vel = RewTerm(
        ...
        weight=-1e-5, # Increased penalty (from -1e-6)
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    ...
    dof_acc = RewTerm(
        ...
        weight=-2.5e-7, # Increased penalty (from -1e-9)
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    ...
    undesired_contacts = RewTerm(
        func=mdp.undesired_contacts,
        weight=-2.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names="Trunk|.*_hip_.*|.*_knee_.*|.*_shank_link|AL.*|AR.*|.*_hand_.*"), "threshold": 1.0},
    )
...
class LeggedRobotCfgPPO: # Removed inheritance from ManagerBasedRLEnvCfg as it's a config class for the runner
    ...
    class runner:
        ...
        # load and resume
        resume = True
        load_run = "/root/logs/rsl_rl/dribbling_g1/2025-12-11_18-01-51" # -1 = last run
        checkpoint = "model_1300.pt" # -1 = last saved model
